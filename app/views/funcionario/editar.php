...
<body>
    <header><?php include_once __DIR__ . '/../includes/menuFuncionario.php'; ?></header>

    <main>
        <div class="container">
            <div id="topo">
                <a href="<?= URL_BASE ?>/funcionario/inicial" class="botao brilho">Voltar</a>
                <div>
                    <h1>Editar Funcionário</h1>
                    <p class="mensagem">Atualize os seus dados.</p>
                </div>
            </div>

            <form class="card" action="<?= URL_BASE ?>/funcionario/atualizar" method="post">
                ...
                <?php foreach ([
                    ...
                    <div class="formgroup">
                        <input class="card-secundario" type="hidden" min="0" id="<?= $campo ?>" name="<?= $campo ?>" value="<?= htmlspecialchars($funcionario[$campo] ?? '0') ?>">
                        <?php if (isset($erros[$campo])): ?>
                            <div class="erro"><?= htmlspecialchars($erros[$campo]) ?></div>
                        <?php endif; ?>
                    </div>
