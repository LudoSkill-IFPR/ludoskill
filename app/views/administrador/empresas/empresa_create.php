...
<head>
    ...
    <title>LudoSkill - Cadastrar Empresa</title>
</head>
...
<body>
    ...
    <main>
        <div class="container">
            <div id="topo"><a class="botao brilho" href="<?= URL_BASE ?>/administrador/empresas">Voltar</a>
                <div>
                    <h1>Cadastro de Empresa</h1>
                    <p class="mensagem">Cadastre uma nova empresa no LudoSkill.</p>
                </div>
            </div>
            <form class="card" action="<?= URL_BASE ?>/administrador/empresas/salvar" method="post">
                ...
                <div class="formgroup"><label for="plano">Plano:</label><select class="card-secundario" id="plano" name="plano">
                        <option value="">Selecione um plano</option><?php foreach (['BASICO' => 'Básico', 'INTERMEDIARIO' => 'Intermediário', 'AVANCADO' => 'Avançado', 'CORPORATIVO' => 'Corporativo'] as $valor => $rotulo): ?><option value="<?= $valor ?>" <?= ($empresa['plano'] ?? '') === $valor ? 'selected' : '' ?>><?= $rotulo ?></option><?php endforeach; ?>
                    </select><?php if (isset($erros['plano'])): ?><div class="erro"><?= htmlspecialchars($erros['plano']) ?></div><?php endif; ?></div>
